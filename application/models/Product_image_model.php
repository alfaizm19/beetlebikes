...
{
	public $table = 'product_image'; // you MUST mention the table name
	...
	public function get_datatables()
	{
		...
		$product_image = array(
			...
			'formatter' => function($image, $row) {
				$getData = $this->db->get_where('product_image', array('id' => $row['id']))->row();
                if ($getData->type == 'image') {
                	return get_image_path($image, $width=25); 
                } else {
                	return '<div class="text-center"><a class="btn btn-primary" target="_blank" href="'.$getData->video_url.'">Video</a></div>';
                }
            }
        );
		...
		$get_edit = array(
			...
			'formatter' => function($id) {
				$id = hash('SHA256', $id);
				return get_edit($id,'admin/product_image/edit/'.$this->uri->segment(4)); /// $id = row_id , method path 
			}
		);
		...
		$data_table = array_values(compact('delete_all','product_image','get_edit','delete'));
