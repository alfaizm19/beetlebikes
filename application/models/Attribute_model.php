...
class Attribute_model extends MY_Model {
    ...
    public $table = 'attribute_name'; // you MUST mention the table name
    ...
    function get_datatables() {
        ...
        $table = 'attribute_name'; // you MUST mention the table name
        ...
        $delete_all = array(
            'customfilter' => 'attribute_name.id',
            'db' => 'attribute_name.id',
        ...
        $attribValue = array(
            ...
            'formatter' => function($id, $row) {
                $id = hash('SHA256', $id);
                return '<a href="'.base_url('admin/attribute-value/'.$id).'">Configure Value</a>';
            }
        );
        ...
        $get_edit = array(
            ...
            'formatter' => function($id) {
                $id = hash('SHA256', $id);
                return get_edit($id, 'admin/attribute/edit'); /// $id = row_id , method path
            }
        );
        ...
        if (DISPLAY_ORDER) {
            $data_table = array_values(compact('delete_all', 'title', 'slug', 'attribValue', 'display_order', 'is_active', 'get_edit', 'delete'));
        } else {
            $data_table = array_values(compact('delete_all', 'title', 'slug', 'attribValue', 'display_order', 'is_active', 'get_edit', 'delete'));
        }
        ...
        return json_encode(
            Datatables_ssp::simple($_GET, $this->dbConn, $table, $primary_key, $columns, $myjoin, $where, 'ORDER BY attribute_name.id desc')
        );
    }
