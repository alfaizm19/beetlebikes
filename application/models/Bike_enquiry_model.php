...
class Bike_enquiry_model extends MY_Model {

    public $table = 'register_bike'; // you MUST mention the table name
    ...
    function get_datatables($filter) {
        ...
        $table = 'register_bike'; // you MUST mention the table name
        $primary_key = 'register_bike.id';
        $where = "1=1 ";

        if (!empty($from_date) && !empty($to_date)) {
            $where .= " AND DATE(register_bike.created_at) >= '".$from_date."' AND DATE(register_bike.created_at) <= '".$to_date."' ";
        } elseif (!empty($from_date) && empty($to_date)) {
            $where .= " AND DATE(register_bike.created_at) >= '".$from_date."' ";
        } elseif (!empty($to_date) && empty($from_date)) {
            $where .= " AND DATE(register_bike.created_at) >= '".$to_date."' ";
        }

        $order = 'ORDER BY register_bike.id DESC';

        $delete_all = array(
            'customfilter' => 'register_bike.id',
            'db' => 'register_bike.id',
        ...
        $date = array(
            'customfilter' => 'register_bike.created_at',
            'db' => 'register_bike.created_at',
        ...
        $get_view = array(
            ...
            'formatter' => function($id) {
                $id = hash('SHA256', $id);
                return get_view($id,'admin/bike_enquiry/enquiry_view');
            }
        );
    ...
    public function get_enquiry_info($id) {
        return $this->db
        ->get_where('register_bike as a', array('SHA2(a.id, 256) = ' => $id))
        ->row();
    }
