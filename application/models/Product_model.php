...
class Product_model extends MY_Model {
    ...
    function get_datatables() {
        ...
        // $myjoin = "LEFT JOIN product_image ON product.id = product_image.product_id ";
        $myjoin = '';
        $custom_where = "";

        $delete_all = array(
            'customfilter' => 'product.id',
            'db' => 'product.id',
        ...
        $product_name = array(
            'customfilter' => 'product_name',
            'db' => 'product_name',
        );

        $sku_code = array(
            'customfilter' => 'sku_code',
            'db' => 'sku_code',
        );

        // $gallery = array('customfilter' => 'product.product_name',
        //     'db' => 'count(product_image.product_id) as product_count', 'dt' => 3,
        //     'formatter' => function( $total_count, $row ) {
        //         $product_image_count = array('id' => $row['id'], 'image_count' => $row['product_count'], 'url_path' => 'admin/product_image/index/');
        //         return get_total_image_count($product_image_count);
        //     }
        // );

        $gallery = array(
            'customfilter' => 'product.product_name',
            'db' => 'product.product_name',
            'dt' => 3,
            'formatter' => function( $id, $row ) {

                $count = $this->db->get_where('product_image', array(
                    'product_id' => $row['id']
                ))->num_rows();

                $id = hash('SHA256', $row['id']);

                return "<div class='text-center'><a style='position: relative;' href='" . base_url() . "admin/product_image/index/" . $id . "'><img class='fa fa-fw' src='" . base_url() . "assets/admin/images/camera38.png' style='width: 35px;'/> <span class='badge'>" . $count . "</span></a></div>";
            }
        );
        ...
        $showcase = array(
            ...
            'formatter' => function( $id, $row ) {
                ...
                $count = $this->db->get_where('product_showcase', array(
                    'product_id' => $row['id']
                ))->num_rows();

                $id = hash('SHA256', $row['id']);

                return "<div class='text-center'><a style='position: relative;' href='" . base_url() . "admin/product_showcase/index/" . $id . "'><img class='fa fa-fw' src='" . base_url() . "assets/admin/images/camera38.png' style='width: 35px;'/> <span class='badge'>" . $count . "</span></a></div>";
            }
        );

        $reviews = array(
            'customfilter' => 'product.image_path',
            'db' => 'product.image_path',
            'formatter' => function( $id, $row ) {

                // $count = $this->db->get_where('product_reviews', array('product_id' => $row['id']))->num_rows();

                $count = $this->db->get_where('reviews', array('product_id' => $row['id']))->num_rows();

                $id = hash('SHA256', $row['id']);

                return "<div class='text-center'><a style='position: relative;' href='" . base_url() . "admin/reviews/index/" . $id . "'><img class='fa fa-fw' src='" . base_url() . "assets/admin/images/reviews.png' style='width: 35px;'/> <span class='badge'>" . $count . "</span></a></div>";
            }
        );
        ...
        $stock = array(
            ...
            'formatter' => function( $id, $row ) {
                ...
                return '<div class="m-form__control form-inline w-75 m-auto">
                 <input type="text" class="form-control w-50 text-center" maxlength="2" value="'.$row["stock"].'" name="stock[]" id="stock" onkeypress="return numericOnly(this);" aria-describedby="stock-error" aria-invalid="false"><div id="stock-error" class="form-control-feedback" style="display: block;"></div>
                </div>';
            }
        );
        ...
        $view = array(
            ...
            'formatter' => function( $id, $row ) {
                $id = hash('SHA256', $id);
                return get_view($id, 'admin/product/view_product');
            }
        );
        ...
        $get_edit = array(
            ...
            'formatter' => function($id) {
                $id = hash('SHA256', $id);
                return get_edit($id, 'admin/product/edit'); /// $id = row_id , method path
            }
        );

        $display_order = array(
            'customfilter' => 'product.display_order',
            'db' => 'product.display_order',
        ...
        $is_active = array(
            'customfilter' => 'product.is_active',
        ...
        $data_table = array_values(compact('delete_all', 'product_name', 'sku_code', 'gallery', 'showcase', 'reviews', 'stock', 'display_order', 'is_active', 'view', 'get_edit', 'delete'));
...
// error_log(json_encode($_GET));
    ...
    public function remove_image($data) {
        ...
        $this->db->where_in('product_id', $data);
        $this->db->delete('product_showcase');
