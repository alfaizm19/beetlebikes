...
class Contact_enquiry extends Admin_Controller {
    
    public function __construct() {
        parent::__construct();
        $this->load->model('Contact_enquiry_model', 'contact');
    }
    ...
    public function index() {        
        ...
        $this->setPageTitle('Manage Contact Enquiry');
        $this->data['soringCol'] = '"order": [[ 3, "desc" ]],';

        $this->data['manage_view_path'] = '' . base_url() . 'admin/contact_enquiry/view/'.urlencode(serialize($filtersData));
        $this->render('admin/vwManageContactEnquiry');
    }
    ...
    public function view($filtersData) {
        ...
        echo $list = $this->contact->get_datatables($filtersData);
    }
    ...
    public function enquiry_view($id = NULL) {
        $this->setPageTitle('Manage Contact Enquiry | View Enquiry Detail');
        ...
        $this->data['info'] = $this->contact->get_enquiry_info($id);
        
        $this->render('admin/vwViewContactEnquiry');
    }
    ...
    public function createxls() {
        ...
        // create file name
        $fileName = 'contact-data-'.time().'.xlsx';
        ...
        if (!empty($info)) {
            ...
            foreach ($info as $inf) {
                ...
                $sheet->SetCellValue('C' . $rowCount, $inf->city);
                $sheet->SetCellValue('D' . $rowCount, $inf->phone);
                $sheet->SetCellValue('E' . $rowCount, $inf->message);
                $sheet->SetCellValue('F' . $rowCount, date('d-m-Y h:i:s A', strtotime($inf->created_at)));
                
                $rowCount++;
            }

            $sheet->setTitle('Exported Contact Enquiry');
