...
class Product_showcase extends Admin_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->library('image_lib');
        $this->load->model('Product_showcase_model', 'product_showcase');
    }
    ...
    public function index($product_id) {
        ...
        $this->data['manage_view_path'] = '' . base_url() . 'admin/product_showcase/view/' . $product_id;
        $this->render('admin/vwManageProductShowcase');
    }
    ...
    public function view($product_id) {
        ...
        $this->setPageTitle('Manage Product Showcase');
        echo $list = $this->product_showcase->get_datatables();
    }
    ...
    public function create($product_id = "") {
        ...
        $this->setPageTitle('Add Product Showcase');
        if ($this->input->post('Submit') === "Save" && $product_id != "") {
            $this->save($product_id);
        }
        $this->render('admin/vwAddProductShowcase');
    }
    ...
    public function save($productId) {
        ...
        if (!empty($isExist)) {
            ...
            if ($type == 'video') {
                ...
                for ($i = 1; $i <= $total_addfield_count; $i++) {
                    ...
                    if ($_FILES['product_image'.$i]['size']) {
                        ...
                        $this->images->setAllowedTypes('product_image' . $i, 'jpg|jpeg|png')->setPath('product_image' . $i, './uploads/productshowcase/');
                        ...
                        $insert = $this->product_showcase->insert($data);
                    }
                }
            ...
            set_flash('message', 'success', 'Product Showcase has been added successfully.');

            redirect('admin/product_showcase/index/' . $this->input->post('hid_product_id') . '', 'refresh');
    ...
    public function edit($productId = NULL, $productImageId = NULL) {
        $this->data['is_CKEditor'] = FALSE;
        $this->setPageTitle('Edit Product Showcase');
        
        $this->data['product'] = $this->product_showcase->get(array('SHA2(id, 256) =' => $productImageId));

        if ($this->input->post('Submit') === "Save" && $productId != "" && $productImageId != "") {
            $this->update($productId, $productImageId);
        }
        $this->render('admin/vwEditProductShowcase');
    }
    ...
    public function update($productId, $id) {
        ...
        if ($type == 'video') {
            ...
            $this->images->setAllowedTypes('product_image', 'jpg|jpeg|png')->setPath('product_image', './uploads/productshowcase/')->setUnlink('product_image', $this->data['product']->image_path);
        ...
        $update = $this->product_showcase->update($data, array('SHA2(id, 256) = ' => $this->uri->segment(5)));

        if ($update) {
            set_flash('message', 'success', 'Product Showcase has been updated successfully.');
            redirect('admin/product_showcase/index/' . $this->input->post('hid_product_id') . '', 'refresh');
        }
        return FALSE;
    }
    ...
    public function delete() {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$perm) {
                ...
                $is_delete = $this->product_showcase->delete($data);

                if ($is_delete) {
                    $this->output->set_content_type('application/json')->set_output(json_encode(array(
                        "success" => true,
                        "message" => "Product Showcase has been deleted successfully.",
                        "type" => "success"
                    )));
                } else {
                    $this->output->set_content_type('application/json')->set_output(json_encode(array(
    ...
    public function bulk_delete() {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$perm) {
                ...
                /** remove image */
                $is_delete = $this->product_showcase->remove_image($data, TRUE);

                /** delete all recode */
                $this->db->where_in('id', $data);
                $is_delete = $this->db->delete('product_showcase');
