...
class Bike_enquiry extends Admin_Controller {
    
    public function __construct() {
        parent::__construct();
        $this->load->model('Bike_enquiry_model', 'bike');
    }
    ...
    public function index() {        
        ...
        $this->setPageTitle('Manage Register Bike Enquiry');
        $this->data['soringCol'] = '"order": [[ 3, "desc" ]],';

        $this->data['manage_view_path'] = '' . base_url() . 'admin/bike_enquiry/view/'.urlencode(serialize($filtersData));
        $this->render('admin/vwManageBikeEnquiry');
    }
    ...
    public function view($filtersData) {
        ...
        echo $list = $this->bike->get_datatables($filtersData);
    }

    public function enquiry_view($id = NULL) {
        $this->setPageTitle('Manage Register Bike Enquiry | View Enquiry Detail');
        ...
        $this->data['info'] = $this->bike->get_enquiry_info($id);
        
        $this->render('admin/vwViewBikeEnquiry');
    }
    ...
    public function createxls() {
        ...
        // create file name
        $fileName = 'bike-enquiry-data-'.time().'.xlsx';

        if (!$perm) {
            set_flash('error', 'danger', 'You dont have permission');
            redirect(base_url('admin/bike_enquiry'),'refresh');
        }
        ...
        $info = $this->db
        ->order_by('a.id', 'desc')
        ->get('register_bike as a')->result_object();        
        ...
        if (!empty($info)) {
            ...
            foreach ($info as $inf) {
                ...
                $sheet->SetCellValue('B' . $rowCount, $inf->phone);
                $sheet->SetCellValue('C' . $rowCount, $inf->email);
                $sheet->SetCellValue('D' . $rowCount, date('d-m-Y', strtotime($inf->invoice_date)));
                $sheet->SetCellValue('E' . $rowCount, $inf->model);
                $sheet->SetCellValue('F' . $rowCount, $inf->chassis_no);
                $sheet->SetCellValue('G' . $rowCount, $inf->city);
                $sheet->SetCellValue('H' . $rowCount, $inf->state);
                $sheet->SetCellValue('I' . $rowCount, date('d-m-Y h:i:s A', strtotime($inf->created_at)));
                
                $rowCount++;
            }

            $sheet->setTitle('Exported Register Bike Enquiry');
