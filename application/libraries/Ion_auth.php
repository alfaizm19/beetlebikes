...
class Ion_auth {
    ...
    public function forgotten_password($identity) {
        if ($this->ion_auth_model->forgotten_password($identity)) {
            ...
            $user = $this->where($identifier, $identity)->where('is_active', 1)->users()->row();
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $this->c_email->setTo($user->email)->setFrom($this->config->item('admin_email', 'ion_auth'));

                    $data_replace = array('uid' => base_url() . 'admin/reset-password/' . $forgotten_password_code, 'username' => $user->username, 'date' => date("F j, Y"), 'project_name' => $this->data['setting']->site_project_name, 'site_url' => base_url(), 'site_logo' => $this->data['setting']->website_admin_logo, 'copyright' => $this->data['setting']->site_copy_right);

                    $this->c_email->setMessag(1)->composeEmail($data_replace);
    ...
    public function forgotten_password_complete($code) {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $this->email->subject($this->lang->line('email_new_password_subject'));
